...
export class ReadSessionsService {
  ...
  async setSlate(sessionId: string, bookIds: string[]) {
    ...
    const blocked = books.find((book) => book.status === "removed");
    if (blocked) {
      throw new BadRequestException("Removed books cannot be nominated");
    }
  ...
  async setProgress(
    ...
    if (!current.session.readers.some((reader) => reader.userId === userId)) {
    ...
    const updated = await this.sessions.updateProgress(
      ...
      userId,
      {
        ...
        isCompleted,
        startedAt: current.progress?.startedAt ?? now,
        completedAt: isCompleted
          ? (current.progress?.completedAt ?? now)
          : null,
      },
    );
    ...
    await this.completeIfDue(current.session.id, now);
    return updated;
  }
  ...
  async createReview(
    ...
    const finished = await this.sessions.findCompletedProgressForBook(
      userId,
      input.bookId,
    );
    if (!finished) {
      throw new ConflictException("Finish the book before reviewing");
    }
